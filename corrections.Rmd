...
em_runs <- BIOMOD_EnsembleModeling(model_runs, # Objet issu de l'etape 2
                                   ...
                                   EMci.alpha = 0.05, # Seuil de l'IC (0.05 pour avoir l'IC 95%)
)
...
```{r}
projection_current <- BIOMOD_Projection(bm.mod = model_runs, # Objet issu de l'etape 2 (calibration des modeles individuels)
...
ef_current <- BIOMOD_EnsembleForecasting(bm.em = em_runs,  # Objet issu de l'etape 3 (ensemble modelling)
...
projection_futuressp126 <- BIOMOD_Projection(bm.mod = model_runs, # Objet issu de l'etape 2 (calibration des modeles individuels)
...
ef_futuressp126 <- BIOMOD_EnsembleForecasting(bm.em = em_runs,  # Objet issu de l'etape 3 (ensemble modelling)
...
projection_futuressp585 <- BIOMOD_Projection(bm.mod = model_runs, # Objet issu de l'etape 2 (calibration des modeles individuels)
...
ef_futuressp585 <- BIOMOD_EnsembleForecasting(bm.em = em_runs,  # Objet issu de l'etape 3 (ensemble modelling)
...
suitability <- c(current_projection[[1]], 
                 futuressp126[[1]],
                 futuressp585[[1]])
...
pa <- c(current_binary[[1]], 
        futuressp126_binary[[1]],
        futuressp585_binary[[1]])
